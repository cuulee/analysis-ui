...
import {selectOpportunitiesUrl, selectOpportunityDatasets} from './selectors'
...
export const downloadOpportunityDataset =
  (dataset: OpportunityDataset, format: string): ThunkAction =>
    (dispatch: Dispatch, getState: GetState) => {
      ...
      const regionId = selectCurrentRegionId(state, {})
      dispatch(fetch({
        url: `/api/opportunities/${regionId}/${dataset.key}/${format}?redirect=false`,
        next (res) {
...
export const loadOpportunityDataset =
  (dataset: PartialOpportunityDataset): ThunkAction =>
    (dispatch: Dispatch, getState: GetState) => {
      const state = getState()
      const regionId = selectCurrentRegionId(state, {})
