...
type Props = {
  ...
  setProfileRequest: (profileRequest: ProfileRequest) => void,
...
export default class SinglePointAnalysis extends Pure {
  ...
  _updateProfileRequest = (updatedFields: any) => {
  ...
  setActiveVariant = (option: ReactSelectResult) => {
    this._updateProfileRequest({
      variantIndex: parseInt(option.value)
    })
  }
  ...
  _setTravelTimePercentile = (e: Event & {target: HTMLInputElement}) =>
    this._updateProfileRequest({travelTimePercentile: Number(e.target.value)})
        ...
        <InnerDock>
          ...
          <div className='block'>
            ...
            <ProfileRequestEditor
              disabled={disableInputs}
              profileRequest={profileRequest}
              updateProfileRequest={this._updateProfileRequest}
            />
            ...
            <Slider
              disabled={disableInputs}
              value={profileRequest.travelTimePercentile}
              output
              format='.1f'
              min={1}
              max={99}
