...
export default class RegionalAnalysisSelector extends Component {
  ...
  state = {
    sortedAnalyses: sortBy(this.props.allAnalyses || [], 'name') // TODO sort in selector
  }
  ...
  componentWillReceiveProps (nextProps: Props) {
    ...
    if (allAnalyses !== this.props.allAnalyses) {
      this.setState({
        sortedAnalyses: sortBy(allAnalyses || [], 'name')
      })
    }
  }
        ...
        {sortedAnalyses
          ...
          .map(a => (
            <Analysis
              analysis={a}
              key={`analysis-${a._id}`}
              onDelete={this._deleteRegionalAnalysis(a._id)}
              onSelect={this._selectRegionalAnalysis(a._id)}
            />
          ))}
      </ul>
    )
