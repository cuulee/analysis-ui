...
export async function atStart ({
  ...
  // insert at start
  let stopAtEnd, to, toStopId

  // handle case of no existing stops
  if (segments.length > 0) {
    to = segments[0].geometry.type === 'LineString'
      ? segments[0].geometry.coordinates[0]
      : segments[0].geometry.coordinates
  ...
  if (segments.length === 2 && segments[1].geometry.type === 'Point') {
    segments.pop()
  }
...
export async function atEnd ({
  ...
  // insert at end
  let from, fromStopId, stopAtStart

  const lastSegIdx = segments.length - 1

  // handle creating the first stop. Note that we only support this when adding at the end.
  if (segments.length > 0) {
    from = segments[lastSegIdx].geometry.type === 'LineString'
      ? segments[lastSegIdx].geometry.coordinates.slice(-1)[0]
      : segments[lastSegIdx].geometry.coordinates
  ...
  // if there was a segment that was just a point, get rid of it
  if (segments.length === 2 && segments[0].geometry.type === 'Point') {
    segments.shift()
  }
